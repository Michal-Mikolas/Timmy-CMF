...
class Todo extends Object implements IBarPanel
{
	...
	public $todoMask = array('TO\s?DO', 'FIX\s?ME', 'PENDING', 'XXX', 'FIX', 'TODO:');
	 ...
	 */
	public function __construct($basedir = NULL, $ignoreMask = array('.git', '.svn', 'cache', 'log', 'sessions', 'temp'))
	{
		if (is_array($basedir)) {
	...
	{
		ob_start();
		$template = new FileTemplate(dirname(__FILE__) . '/TodoPanel.latte');
	...
	{
		Debugger::addPanel(new self($basepath));
	}
